...
function Contact(){
    ...
    return <div id="contact">
        ...
        <form ref={form} onSubmit={sendEmail} className="contactForm">
            ...
            type="text" name="name" placeholder="Name" value={name_val} onChange={(e)=>{
                const new_name = e.target.value;
                setNameVal(new_name);
            }}/>
            <input 
            className="formField" required
            type="email" name="email" placeholder="Email" value={email_val} onChange={(e)=>{
                const new_email = e.target.value;
                setEmailVal(new_email);
            }}/>
            <textarea 
            className="formTextArea"  required
            name="message" placeholder="Message" value={msg_val} onChange={(e)=>{
                const new_msg = e.target.value;
                setMsgVal(new_msg);
            }}/>
            
            <div className="formBtnDiv">
                <button className="formBtn" type="submit" value="send">Send</button>
            </div>
        </form>
        <span>
            <ArrowCircleUpIcon className="scrollTop" onClick={()=>{ 
                scroll.scrollToTop();
            } }/>
        </span>
    </div>
